...
function tagsToRole(tags=[]) {
  ...
  if (tags.includes('Support') && !tags.includes('Fighter') && !tags.includes('Mage')) return 'SUPPORT'
  if (tags.includes('Tank') || tags.includes('Fighter')) return 'TOP'
  if (tags.includes('Assassin') || tags.includes('Mage')) return 'MID'
  return null
}
function detectRole(s1,s2,tagRole) {
  const sp=[s1,s2],isM=tagRole==='ADC',isS=tagRole==='SUPPORT'
  if(sp.includes(11)) return {role:'JUNGLE',conf:100}
  if(sp.includes(3)&&!isM) return {role:'SUPPORT',conf:98}
  if(sp.includes(7)){if(isM)return{role:'ADC',conf:99};if(isS)return{role:'SUPPORT',conf:85};return{role:'ADC',conf:72}}
  if(sp.includes(21)){if(isS)return{role:'SUPPORT',conf:80};if(isM)return{role:'ADC',conf:75};return{role:null,conf:0}}
  if(sp.includes(14)&&isS)return{role:'SUPPORT',conf:82}
  if(sp.includes(12))return{role:null,conf:0,hint:'TP'}
  if(sp.includes(14))return{role:null,conf:0,hint:'IGN'}
  return{role:null,conf:0}
}
function resolveAdcSupport(players,assigned,usedRoles){
  if(usedRoles.has('ADC')&&usedRoles.has('SUPPORT'))return
  if(usedRoles.has('ADC')||usedRoles.has('SUPPORT')){
    const need=usedRoles.has('ADC')?'SUPPORT':'ADC'
    for(let i=0;i<players.length;i++){if(!assigned.has(i)){assigned.set(i,need);usedRoles.add(need);return}}
    return
  }
  const un=players.map((p,i)=>({p,i})).filter(({i})=>!assigned.has(i))
  if(un.length<2)return
  let best=-1,bA=-1,bS=-1
  for(let a=0;a<un.length;a++)for(let b=0;b<un.length;b++){
    if(a===b)continue
    const{p:pa,i:ia}=un[a],{p:pb,i:ib}=un[b]
    const{role:ra,conf:ca}=detectRole(pa.spell1Id,pa.spell2Id,pa._tagRole)
    const{role:rb,conf:cb}=detectRole(pb.spell1Id,pb.spell2Id,pb._tagRole)
    let sc=0
    if(ra==='ADC')sc+=ca;if(rb==='SUPPORT')sc+=cb
    if(pa._tagRole==='ADC')sc+=20;if(pb._tagRole==='SUPPORT')sc+=20
    if(sc>best){best=sc;bA=ia;bS=ib}
  }
  if(bA!==-1){assigned.set(bA,'ADC');usedRoles.add('ADC')}
  if(bS!==-1){assigned.set(bS,'SUPPORT');usedRoles.add('SUPPORT')}
}

function useLiveTimer(init){
  const[s,setS]=useState(init??0);const ref=useRef(null);const cur=useRef(init??0)
  useEffect(()=>{if(init==null)return;if(Math.abs(init-cur.current)>5){setS(init);cur.current=init};if(ref.current)return;ref.current=setInterval(()=>{cur.current+=1;setS(x=>x+1)},1000);return()=>{clearInterval(ref.current);ref.current=null}},[init])
  return s
}
...
const normR = r=>r?(ROLE_LABELS[r.toUpperCase()]??null):null
...
// ─── Draft composants (identiques à l'original) ──────────────
function SplashCell({player,side}){
  const splash=cSpl(player?.championName)
  return(
    <div className="gp-splash-cell">
      {splash?<img src={splash} alt={player?.championName} className="gp-splash-img" onError={e=>{e.target.style.display='none'}}/>:<div className="gp-splash-placeholder">{player?.championName?.slice(0,2)??'?'}</div>}
...
function PlayerRow({player,side}){
  ...
  return(
    <div className="gp-player-row">
      <div className="gp-player-pseudo">{player?.pro&&<span className="gp-pro-badge">{player.pro.team}</span>}{player?.summonerName||'—'}</div>
      <div className="gp-player-name">{player?.championName||'?'}</div>
      <div className={`gp-player-role ${side}`}><span>{ROLE_ICONS[role]??'❓'}</span><span>{role}</span></div>
    </div>
  )
}
        ...
        <div className="gp-picker-sides">
          {[{team:blue,side:'blue'},{team:red,side:'red'}].map(({team,side})=>(
            <div key={side} className="gp-picker-side">
              ...
              {team.map((p,i)=>{
                const name=p.championName,isSel=selected===name
                return(
                  <button key={i} className={`gp-picker-btn gp-picker-${side}${isSel?' sel':''}`} onClick={()=>onSelect(name)}>
                    <div className="gp-picker-icon">{cIcon(name)?<img src={cIcon(name)} alt="" onError={e=>{e.target.style.display='none'}}/>:<span>{name?.slice(0,2)}</span>}</div>
...
export default function Game(){
  ...
  useEffect(()=>{
    fetch(`https://ddragon.leagueoflegends.com/cdn/${DDV}/data/en_US/champion.json`)
      .then(r=>r.json()).then(d=>{const m={},tm={};Object.values(d.data).forEach(c=>{m[String(parseInt(c.key))]=c.id;tm[c.id]=tagsToRole(c.tags)});setChampMap(m);setChampTagMap(tm)}).catch(()=>{})
  },[])

  const loadGame=useCallback(()=>{api.get(`/games/${id}`).then(r=>{setGame(r.data);setLoading(false)}).catch(()=>{setError('Partie introuvable');setLoading(false)})},[id])
  ...
  const setDur=slug=>{
    if(!betsOpen)return
    setSelections(prev=>{const n={...prev};DURATION_SLUGS.forEach(s=>Object.keys(n).filter(k=>k.startsWith(s+'::')).forEach(k=>delete n[k]));const k=`${slug}::confirmed`;n[k]?delete n[k]:n[k]={slug,value:'confirmed'};return n})
  }
  ...
  const handleBet=async()=>{
    ...
    try{
      let last=null
      for(const s of selList){const r=await api.post('/bets/place',{live_game_id:game.id,bet_type_slug:s.type,bet_value:s.value,amount:parseInt(amount)});last=r.data}
      setBetDone(last);if(last?.coins_restants!==undefined)updateCoins(last.coins_restants);setSelections({});setAmount('')
    }catch(e){setBetError(e.response?.data?.detail||'Erreur lors du pari')}
  ...
  const PickBtn=({slug,title})=>{
    ...
    return(
      <button className="gp-pick-btn" onClick={()=>setPicker({slug,title})} disabled={!betsOpen}>
        {val?<><div className="gp-pick-icon">{cIcon(val)&&<img src={cIcon(val)} alt="" onError={e=>{e.target.style.display='none'}}/>}</div><span className="gp-pick-val">{val}</span><button className="gp-pick-clear" onClick={e=>{e.stopPropagation();toggle(slug,val)}}>✕</button></>:<span>Choisir →</span>}
      </button>
    )
  }
